document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      // Collect non-required field data
      const allInputs = contactForm.querySelectorAll('input, textarea, select');
      allInputs.forEach(input => {
        if (input.name && !input.hasAttribute('required')) {
  ...
  function showError(field, message) {
    ...
    errorDiv.style.cssText = 'color: red; font-size: 0.875rem; margin-top: 4px;';
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '4f6cb7cc-96e2-4604-9223-fb6bd7246c3c',
                    name: data.name || '',
                    email: data.email || '',
